...
############################### for front -end #####################################
# @st.cache_data
def agent(query: str, thread_id: int):
    ...
    user_query = query  # I can replace this input by the input function by the UI 
    ...
    if not flag:
        ...
        response = processing_agent_response(messages, query)
...
async def chat_bot_BL(data:QueryData):
    ...
    '''
    # Session ID
    t_id = data.thread_id;  # from the ui  
